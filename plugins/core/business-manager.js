export default {
  ...
  run: async (conn, m) => {
    ...
    if (m.command === "addcatalog") {
      // Format: addcatalog nama|deskripsi|harga|mata_uang
      // Gambar diambil dari media yang di-reply
    ...
    if (m.command === "updatecatalog") {
      // Format: updatecatalog idProduk|field=value|field=value
      ...
      if (!id || !fields.length) {
        return m.reply(
          "Format salah.\n\n" +
            ...
            "Contoh:\n" +
            "   updatecatalog 123456|name=Kaos Baru|price=85000",
        );
      }
      ...
      // Ganti gambar jika ada media yang di-reply
      if (m.quoted?.isMedia) {
        const buffer = await m.quoted.download();
